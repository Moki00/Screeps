...
const IVM =
  typeof Game.cpu.getHeapStatistics === "function" &&
...
class QosKernel {
  ...
  start() {
    if (IVM) {
      Logger.log(
        `Initializing Kernel for tick ${Game.time} with IVM support`,
        LOG_TRACE,
        "kernel"
      );
    } else {
      Logger.log(
        `Initializing Kernel for tick ${Game.time}`,
        LOG_TRACE,
        "kernel"
      );
    }
  ...
  shouldContinue() {
    ...
    if (Game.cpu.bucket <= BUCKET_EMERGENCY) {
      if (
        !Memory.qos.last_build_bucket ||
        Game.time - Memory.qos.last_build_bucket > BUCKET_BUILD_LIMIT
      ) {
  ...
  getCpuLimit() {
    ...
    if (!this._cpuLimit) {
      ...
      if (this.newglobal) {
        ...
        RECURRING_BURST_FREQUENCY &&
        Game.time % RECURRING_BURST_FREQUENCY === 0
      ) {
    ...
    if (Memory.userConfig && Memory.userConfig.terseConsole) {
      ...
      if (IVM) {
        ...
        const heapPercent = Math.round(
          (heap.total_heap_size / heap.heap_size_limit) * 100
        );
        const sizeMB = heap.total_heap_size >> 20;
        const limitMB = heap.heap_size_limit >> 20;
        message += `, H: ${_.padLeft(heapPercent, 2)}% ${_.padLeft(
          `(${sizeMB}/${limitMB}MB)`,
          11
        )}`;
      }
      ...
      if (IVM) {
        ...
        Logger.log(
          `Heap Used: ${heapPercent}% (${heap.total_heap_size} / ${heap.heap_size_limit})`,
          LOG_INFO,
          "kernel"
        );
      }
