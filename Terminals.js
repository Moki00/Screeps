    ...
    run: function () {
        ...
        function GetFactoryResources(toTerminal, terminals, memRoom) {
            if (memRoom && memRoom.FctrId && memRoom.FctrId !== "-") {
                ...
                if (factory) {
                    const resourceTypesNeeded = GetListOfFactoryResources(
                        factory
                    );
                    for (const resourceNeedKey in resourceTypesNeeded) {
                        const resourceTypeNeeded =
                            resourceTypesNeeded[resourceNeedKey];
                        const amountNeeded =
                            Util.TERMINAL_TARGET_RESOURCE -
                            toTerminal.store.getUsedCapacity(
                                resourceTypeNeeded
                            );
                        if (amountNeeded > Util.TERMINAL_BUFFER) {
                            const didSend = GetFromTerminal(
                                ...
                                toTerminal,
                                terminals,
                                GetNeededFactoryLeftoverResource(
                                    resourceTypeNeeded
                                )
                            );
                            if (
                                ...
                                resourceTypeNeeded.length ===
                                    1 /*only buy H, O, L, U, K, Z, X*/
                            ) {
                                // try to buy resource
                                if (
                                    marketDealCount >= 10 ||
        ...
        function GetEnergy(toTerminal, terminals) {
            if (
                ...
                if (
                    toTerminal.room.controller.level === 8 ||
                    memRoom.FctrId !== "-"
                ) {
        ...
        function BuyResources(toTerminal) {
            ...
            if (
                ...
                toTerminal.room.storage &&
                !toTerminal.room.storage.store.getUsedCapacity(
                    RESOURCE_POWER
                ) &&
                toTerminal.room.storage.store.getUsedCapacity(
                    RESOURCE_ENERGY
        ...
        function SendExcess(fromTerminal) {
            ...
            for (const resourceType in fromTerminal.store) {
                ...
                if (resourceType === RESOURCE_ENERGY) {
                    if (
                        fromTerminal.room.storage.store.getUsedCapacity(
                            RESOURCE_ENERGY
                        ) > Util.STORAGE_ENERGY_HIGH
                    ) {
                ...
                if (
                    ...
                    const amount =
                        fromTerminal.store.getUsedCapacity(resourceType) - max;
                    const didSell = TrySellResource(
                        fromTerminal,
                        resourceType,
                        amount
                    );
        ...
        function TryBuyResource(
            ...
            const orders = Game.market.getAllOrders(
                (order) =>
                    ...
                    (!resourceHistory[0] ||
                        IsOutdated(
                            resourceHistory[resourceHistory.length - 1].date
                        ) ||
