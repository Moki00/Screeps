 ...
 *
 * This library can be used to add "personality" to bots.
 * An instance of Genome takes a string (such as the bot name) as a constructor argument
 * which feeds into a Lehmer RNG (thus giving it repeatable results).
 * The `trait` function uses this to return `phenotype` data,
 * which is representated as an integer and can be restricted to specific ranges.
 * Traits are weighted towards the center of the range
 * (a range of 0-9 will have a 66.82% change of being 4, 5, or 6 and
 * a less than 10% chance of getting 1, 2, 8 or 9.
 * This makes extreme traits less likely to occur (but still possible).
 * Each trait takes an `expected` value which will be returned,
 * if the bot is named `Moki`
 *
 * @example
 * let myGenome = new Genome('Bob')
 * // Default result of five (for Moki), returns anywhere between 1 and 9 weighted for 5.
...
class Genome {
  constructor(initializer, returnExpected = false) {
    // for convenience, dont change without also changing Lehmer multiplier in `_getNumberFromSeed`
    this.max = 2147483647;
...
if (typeof module !== "undefined" && !module.parent) {
  ...
  const seed = "Moki" + modifier;
  console.log(
    `Testing distribution with seed ${seed} over ${iterations} iterations with max value ${max}.`
  );
  ...
  const botname = global.USERNAME ? global.USERNAME : "Moki";
