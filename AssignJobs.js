...
const AssignJobs = {
  run: function () {
    ...
    function ShouldSpawnCreep(creepType, roomKey) {
      ...
      return (
        Object.keys(memRoom.MaxCreeps[creepType]).length - 1 < maxCreepsInRoom
      );
    }
    ...
    function FindBestSpawn(
      ...
      for (const availableSpawnCounter in Game.spawns) {
        ...
        if (
          availableSpawn.room.energyCapacityAvailable >= minEnergyCapacityNeeded
        ) {
          if (availableSpawnsInRoom.length > 0) {
            // spawn in room
            ...
            if (distance !== -1 && distance <= timeToLiveMaxRoomRange) {
              ...
              if (
                availableSpawn.room.energyCapacityAvailable / 2 <=
              ...
              if (
                energyAvailableModifier + distance < bestDistance &&
      ...
      if (
        ...
        if (spawnResult === OK) {
          ...
          if (Memory.MemRooms[memRoomKey].MaxCreeps[creepType]) {
            Memory.MemRooms[memRoomKey].MaxCreeps[creepType][availableName] =
              availableName;
          }
          Util.Info(
            "AssignJobs",
