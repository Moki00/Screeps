...
module.exports.loop = function () {
    ...
    function Controller() {
        ...
        if (Game.time % Util.GAME_TIME_MODULO_2 === 0) {
            // tick burst from https://docs.screeps.com/cpu-limit.html#Bucket
                ...
                if (Game.time % Util.GAME_TIME_MODULO_4 === 0) {
                    ...
                    if (Game.time % Util.GAME_TIME_MODULO_5 === 0) {
                        ...
                        if (Game.time % Util.GAME_TIME_MODULO_6 === 0) {
                            ...
                            if (Game.time % Util.GAME_TIME_MODULO_7 === 0) {
                                // approx every 3 days
    ...
    function UnusedRoomsCleanup(memRoomKey, memRoom) {
        if (
            memRoom.RoomLevel <= 0 &&
            Object.keys(memRoom.RoomJobs).length === 0
        ) {
            let foundCreep = false;
            for (const creepType in memRoom.MaxCreeps) {
                const maxCreep = memRoom.MaxCreeps[creepType];
                if (maxCreep && Object.keys(maxCreep).length > 1) {
                    // more than 'M' is present - a creep is still attached to the room. wait until it dies
                    foundCreep = true;
                    break;
                }
            }
            if (!foundCreep) {
                ...
                Util.InfoLog(
                    "Main",
                    "Main",
                    "removed unused room " + memRoomKey
                );
            }
        }
    }
        ...
        for (const memRoomKey in Memory.MemRooms) {
            ...
            if (memRoom) {
                ...
                if (memRoom.IsReserved && memRoom.MainRoom) {
                    ...
                    if (
                        flag &&
                        Memory.MemRooms[memRoom.MainRoom] &&
                        Memory.MemRooms[memRoom.MainRoom].MainSpawnId
                    ) {
                ...
                if (mineral) {
                    const color =
                        mineral === RESOURCE_HYDROGEN
                            ...
                            ? "#ffffff"
                            : mineral === RESOURCE_UTRIUM
                            ? "#00ffff"
                            : mineral === RESOURCE_LEMERGIUM
                            ? "#00ff00"
                            : mineral === RESOURCE_KEANIUM
                            ? "#8000ff"
                            : mineral === RESOURCE_ZYNTHIUM
                            ? "#ffff00"
                            : mineral === RESOURCE_CATALYST
                            ? "#ff0000"
                            : "#000000";
                    Game.map.visual.text(
                        mineral,
                        new RoomPosition(46, 5, memRoomKey),
                        {
                            color: color,
                            fontSize: 7,
                            opacity: 1,
                ...
                Game.map.visual.text(
                    memRoom.RoomLevel,
                    new RoomPosition(4, 5, memRoomKey),
                    {
                        color:
                            memRoom.RoomLevel > 0
                                ? "#00ff00"
                ...
                Game.map.visual.rect(
                    new RoomPosition(0, 0, memRoomKey),
                    50,
                    50,
                    {
                        stroke:
