...
const ExecuteJobs = {
    run: function () {
            ...
            for (const creepName in Memory.creeps) {
                ...
                if (!creepMemory.JobName) {
                    Util.ErrorLog(
                        "ExecuteJobs",
                        "ExecuteRoomJobs",
                        "creep JobName is undefined " + creepName
                    );
                    if (!gameCreep) {
                        Util.ErrorLog(
                            "ExecuteJobs",
                            "ExecuteRoomJobs",
                            "gameCreep is undefined " + creepName
                        );
                        ...
                        Util.Warning(
                            "ExecuteJobs",
                            "ExecuteRoomJobs",
                            "setting undefined JobName to idle " +
                                creepName +
                                " " +
                                gameCreep.pos.roomName
                        );
                        creepMemory.JobName =
                            "idle(" +
                            gameCreep.pos.x +
                ...
                if (
                    !creepMemory.JobName.startsWith("idle") &&
                    Memory.MemRooms[jobRoomName]
                ) {
                    result = CreepActive(
                        jobRoomName,
                        creepMemory,
                ...
                if (result !== OK && gameCreep) {
                    // ConstantCreepActions should mostly be run when a creep is on the move, if it did an action it should not be cancelled by a newer action
        ...
        function CreepActive(
            ...
            const job =
                Memory.MemRooms[jobRoomName].RoomJobs[creepMemory.JobName];
            if (job && gameCreep) {
                // creep is alive and its job is found　仕事を見つけた
                if (!gameCreep.spawning) {
                    result = JobAction(gameCreep, job);
                    if (result === JOB_IS_DONE) {
                        if (
                            Memory.MemRooms[jobRoomName].RoomJobs[
                                creepMemory.JobName
                            ]
                        ) {
                            Util.DeleteJob(
                        ...
                        for (const roomJobKey in Memory.MemRooms[jobRoomName]
                            .RoomJobs) {
                            let roomJob =
                                Memory.MemRooms[jobRoomName].RoomJobs[
                                    roomJobKey
                            ...
                            if (
                                ...
                                creepName.startsWith(roomJob.CreepType)
                            ) {
                ...
                // creep is not able to do the job　仕事できない
                ActiveCreepCleanup(
                    job,
                    gameCreep,
                    creepMemory,
        ...
        function ActiveCreepCleanup(
            ...
            if (!job && gameCreep) {
                // job is outdated and removed from Memory and creep is still alive
                Util.ErrorLog(
                    "ExecuteJobs",
                    " ActiveCreepCleanup",
                    creepName +
                        " job unexpectedly disappeared! " +
                        creepMemory.JobName
                );
                creepMemory.JobName =
                    "idle(" +
                ...
                // creep is dead
                if (job && !gameCreep) {
                    // job exists and creep is dead, remove job
                    Util.DeleteJob(job, creepMemory.JobName, jobRoomName);
                }
                const removedMaxCreepsInRoom = FindAndRemoveMaxCreeps(
                    jobRoomName,
                    creepName
                );
                delete Memory.creeps[creepName];
        ...
        function CreepIdle(jobRoomName, gameCreep, creepName, result) {
            if (!gameCreep) {
                ...
                if (
                    gameCreep.room.storage &&
                    gameCreep.room.storage.store.getUsedCapacity() <
                        gameCreep.room.storage.store.getCapacity() &&
                    gameCreep.store.getUsedCapacity() > 0
                ) {
                    result = DepositCreepStore(
                        gameCreep,
                        gameCreep.room.storage
                    );
                    ...
                    gameCreep.getActiveBodyparts(ATTACK)
                ) {
                    // idle creep can attack
                    result = IdleCreepAttack(gameCreep);
                } else if (
                    result === ERR_NO_RESULT_FOUND &&
                    gameCreep.getActiveBodyparts(RANGED_ATTACK)
                ) {
                    // idle creep can ranged attack
                    result = IdleCreepRangedAttack(gameCreep);
                } else if (
                    result === ERR_NO_RESULT_FOUND &&
                    gameCreep.getActiveBodyparts(HEAL)
                ) {
                    // idle creep can heal
                ...
                if (
                    result === ERR_NO_RESULT_FOUND &&
                    (!gameCreep.room.controller ||
                        !gameCreep.room.controller.my ||
                        gameCreep.memory.MoveHome ||
                        (Memory.MemRooms[gameCreep.pos.roomName] &&
                            Memory.MemRooms[gameCreep.pos.roomName].MaxCreeps[
                                creepName.substring(0, 1)
                            ] &&
                            !Memory.MemRooms[gameCreep.pos.roomName].MaxCreeps[
                                creepName.substring(0, 1)
                            ][creepName]))
                ) {
                    // I do not own the room the idle creep is in - move it to an owned room!
                    result = IdleCreepMoveHome(
                        creepName,
                        gameCreep,
                        jobRoomName
                    );
            ...
            if (hostileCreeps[0]) {
                ...
                Util.Info(
                    ...
                    "idle " +
                        ...
                        " hostile creeps! targeting " +
                        hostileCreep +
                        " attack in " +
                        gameCreep.pos.roomName
                );
                gameCreep.say(
                    "ATK " + hostileCreep.pos.x + "," + hostileCreep.pos.y
                );
        ...
        function IdleCreepRangedAttack(gameCreep) {
            ...
            if (hostileCreeps[0]) {
                ...
                gameCreep.say(
                    "RATK " + hostileCreep.pos.x + "," + hostileCreep.pos.y
                );
            ...
            if (damagedCreeps[0]) {
                ...
                Util.Info(
                    ...
                    "idle " +
                        gameCreep.name +
                        " found " +
                        damagedCreeps.length +
                        " damaged creeps! targeting " +
                        damagedCreep +
                        " heal"
                );
                gameCreep.say(
                    "HEAL " + damagedCreep.pos.x + "," + damagedCreep.pos.y
                );
        ...
        function IdleCreepMoveHome(creepName, gameCreep, jobRoomName) {
            ...
            if (!gameCreep.memory.MoveHome) {
                ...
                if (closestOwnedRoom) {
                    ...
                    if (
                        !Memory.MemRooms[closestOwnedRoom].MaxCreeps[
                            creepName.substring(0, 1)
                        ]
                    ) {
                        Memory.MemRooms[closestOwnedRoom].MaxCreeps[
                    ...
                    Memory.MemRooms[closestOwnedRoom].MaxCreeps[
                        creepName.substring(0, 1)
                    ][creepName] = creepName; // add to the new home room
            ...
            if (
                Game.rooms[closestOwnedRoom] &&
                closestOwnedRoom &&
                (closestOwnedRoom !== gameCreep.pos.roomName ||
                    gameCreep.pos.getRangeTo(
                        Game.rooms[closestOwnedRoom].controller
                    ) > 4)
            ) {
                result = Move(
                    gameCreep,
        ...
        function RecycleIdleCreep(creepName, gameCreep) {
            ...
            if (
                maxCreeps &&
                maxCreeps[creepType] &&
                (Object.keys(maxCreeps[creepType]).length - 1 >
                    maxCreeps[creepType]["M"] ||
                    !maxCreeps[creepType]["M"])
            ) {
                // check if creepType is overrepresented in this room - recycle creep
                const closestSpawn = gameCreep.pos.findClosestByPath(
                    FIND_MY_STRUCTURES,
                    {
                        filter: (s) => {
        ...
        function ConstantCreepActions(creepMemory, gameCreep) {
            let result = ERR_NO_RESULT_FOUND;
            const energyUsers = gameCreep.pos.findInRange(
                FIND_MY_STRUCTURES,
                1,
                {
                    filter: (structure) => {
            ...
            if (gameCreep.store.getUsedCapacity(RESOURCE_ENERGY) > 0) {
                // fill adjacent spawns, extensions and towers or repair or construct on the road
                result = TryFillOrRepairOrBuild(
                    creepMemory,
                    gameCreep,
                    energyUsers,
                    result
            ...
            if (
                !creepMemory.Boost /*do not renew if creep is boosted*/ &&
                600 / gameCreep.body.length + gameCreep.ticksToLive <= 1500
            ) {
                // spawn renew functionality
                result = TryRenewCreepAdjacentToSpawn(
                    creepMemory,
                    gameCreep,
                    energyUsers,
                    result
            ...
            if (
                result === ERR_NO_RESULT_FOUND &&
                gameCreep.store.getUsedCapacity() <
                    gameCreep.store.getCapacity()
            ) {
                // pickup adjacent resources
                result = TryPickupDropOrTombstone(
                    creepMemory,
                    gameCreep,
                    result
                );
        ...
        /**@return {number}*/
        function TryFillOrRepairOrBuild(
            creepMemory,
            gameCreep,
            energyUsers,
            result
        ) {
            const energyUsersNeedsEnergy = energyUsers.filter(
                (structure) =>
                    structure.store.getFreeCapacity(RESOURCE_ENERGY) > 0
            );
            if (energyUsersNeedsEnergy[0]) {
                // fill adjacent spawns, extensions
                result = gameCreep.transfer(
                    energyUsersNeedsEnergy[0],
                    RESOURCE_ENERGY
                ); // it may do that 'double' but it really does not matter
                ...
                gameCreep.name.startsWith("H") ||
                gameCreep.name.startsWith("B") ||
                gameCreep.name.startsWith("D")
            ) {
                // repair on the road
                ...
                if (toRepair) {
                    ...
                    const toBuild = gameCreep.pos.findInRange(
        ...
        function TryPickupDropOrTombstone(creepMemory, gameCreep, result) {
            const drop = gameCreep.pos.findInRange(
            ...
            if (
                ...
                ((!gameCreep.name.startsWith("H") &&
                    !gameCreep.name.startsWith("D")) ||
                    drop.resourceType === RESOURCE_SILICON ||
                    drop.resourceType === RESOURCE_BIOMASS ||
                    drop.resourceType === RESOURCE_METAL ||
                    drop.resourceType === RESOURCE_MIST)
            ) {
                ...
                const tombstone = gameCreep.pos.findInRange(
                    FIND_TOMBSTONES,
                    1,
                    {
                        filter: (t) => {
        ...
        /**@return {number}*/
        function TryRenewCreepAdjacentToSpawn(
            creepMemory,
            gameCreep,
            energyUsers,
            result
        ) {
            const spawns = energyUsers.filter(
                (structure) =>
                    structure.structureType === STRUCTURE_SPAWN &&
                    !structure.spawning
            );
        ...
        function JobAction(creep, roomJob) {
            ...
            if (result === OK) {
                ...
                // results where anything else than OK - one should end the job!
                if (result === ERR_NO_RESULT_FOUND) {
                    ...
                    Util.Warning(
                        "ExecuteJobs",
                        "JobAction",
                        "no fetch object found " +
                            result +
                            " " +
                            jobKey +
                            " " +
                            roomJob.Creep
                    ); // most likely no energy to withdraw
                    creep.say("⚠⚡" + result);
                } else {
                    if (!result) {
                        Util.Info(
                        ...
                        Util.ErrorLog(
                            "ExecuteJobs",
                            "JobAction",
                            "undefined result " + creep.name + " " + jobKey
                        );
                        creep.say("⚠" + result);
        ...
        function JobSource(creep, roomJob) {
            let result = GenericJobAction(creep, roomJob, {
                ...
                IsJobDone: function (jobObject) {
                    if (
                        ...
                        // predict that creep will be full and make a transfer that wont stop the harvesting flow
                        let fetchObject;
                        let result = ERR_NO_RESULT_FOUND;
                        if (creep.memory.LinkId) {
                            fetchObject = Game.getObjectById(
                                creep.memory.LinkId
                            );
                            if (fetchObject) {
                                result = creep.transfer(
                                    fetchObject,
                                    RESOURCE_ENERGY
                                );
                            }
                        ...
                        if (result !== OK) {
                            fetchObject = Game.getObjectById(
                                creep.memory.ClosestFreeStoreId
                            );
                            if (fetchObject) {
                                result = creep.transfer(
                                    fetchObject,
                                ...
                                if (
                                    result === OK &&
                                    (fetchObject.pos.x !== creep.pos.x ||
                                        fetchObject.pos.y !== creep.pos.y)
                                ) {
                ...
                FindFetchObject: function (jobObject) {
                    ...
                    if (isOnlyEnergy) {
                        fetchObject = FindClosestFreeStore(
                            creep,
                            2,
                            creep.store.getUsedCapacity(RESOURCE_ENERGY),
                            RESOURCE_ENERGY
                    ...
                    if (
                        !fetchObject &&
                        Memory.MemRooms[jobObject.room.name].MissingSpawn
                    ) {
                        const spawnConstruction = jobObject.room.find(
                            FIND_MY_CONSTRUCTION_SITES,
                            {
                                // if there is a spawn that should be built - then build it
                                filter: function (c) {
                    ...
                    if (!fetchObject) {
                        const nearbyConstruction = creep.pos.findInRange(
                            FIND_MY_CONSTRUCTION_SITES,
                            2
                        )[0];
                    ...
                    if (!fetchObject) {
                        // nothing can be found then drop
                        fetchObject = "DROP";
                    } else if (
                        !creep.memory.LinkId &&
                        fetchObject.structureType === STRUCTURE_LINK &&
                        creep.pos.getRangeTo(fetchObject.pos) < 2
                    ) {
                        // if fetchObject is link then save in memory
                        creep.memory.LinkId = fetchObject.id;
                    } else if (
                        creep.memory.LinkId &&
                        fetchObject.structureType !== STRUCTURE_LINK
                    ) {
                        // if fetchObject is not link and a link is saved in memory then take that instead
                        const link = Game.getObjectById(creep.memory.LinkId);
                        if (
                            link &&
                            link.store &&
                            link.store.getFreeCapacity(RESOURCE_ENERGY) > 200 &&
                            isOnlyEnergy
                        ) {
                            fetchObject = link;
            ...
            if (result === ERR_NOT_ENOUGH_RESOURCES) {
                // repair when source is empty
                if (!creep.store.getUsedCapacity(RESOURCE_ENERGY)) {
                    const energyDeposit = Game.getObjectById(
                        creep.memory.ClosestFreeStoreId
                    );
                    if (
                        energyDeposit &&
                        energyDeposit.store.getUsedCapacity(RESOURCE_ENERGY)
                    ) {
        ...
        function JobController(creep, roomJob) {
            const result = GenericJobAction(creep, roomJob, {
                 ...
                 * @return {undefined}*/
                FindFetchObject: function (jobObject) {
                    const labThatCanBoostOrUnBoost = HandleCreepBoost(
                        creep,
                        jobObject,
                    ...
                    let energySupply = FindFetchResource(
                        creep,
                        jobObject,
                        RESOURCE_ENERGY
                    );
                ...
                Fetch: function (fetchObject, jobObject) {
                    ...
                    if (fetchObject.structureType === STRUCTURE_LAB) {
                        ...
                        if (
                            fetchObject.energyCapacity &&
                            creep.room.controller &&
                            creep.room.controller.my &&
                            creep.room.controller.level < 3
                        ) {
                            // this is a source - harvest it
        ...
        function JobFillControllerContainer(creep, roomJob) {
            const result = GenericJobAction(creep, roomJob, {
                ...
                IsJobDone: function (jobObject) {
                    if (
                        creep.store.getUsedCapacity(RESOURCE_ENERGY) +
                            jobObject.store.getUsedCapacity(RESOURCE_ENERGY) >=
                        jobObject.store.getCapacity(RESOURCE_ENERGY)
                    ) {
                        return JOB_IS_DONE;
                    } else {
                        return this.JobStatus(jobObject);
        ...
        function JobFillStorage(creep, roomJob) {
            ...
            if (!resourceType) {
                resourceType = creep.memory.JobName.split(/[(,)]+/).filter(
                    function (e) {
                        return e;
                    }
                )[3];
            ...
            const result = GenericJobAction(creep, roomJob, {
                ...
                JobStatus: function (jobObject) {
                    if (!creep.store.getUsedCapacity()) {
                        // if creep is not carrying anything
                        // if the target is a dropped resource it may just disappear because it was picked up
                        if (
                            !jobObject ||
                            (!jobObject.resourceType /*drop*/ &&
                                jobObject.store.getUsedCapacity(resourceType) <=
                                    GetDesiredLeftoverResource(
                                        jobObject.structureType,
                                        resourceType,
                                        jobObject.room.storage
                ...
                FindFetchObject: function (jobObject) {
                    if (jobObject.room && jobObject.room.storage) {
                        ...
                        creep.room.storage &&
                        ((jobObject.room &&
                            jobObject.room.name !== creep.room.name) ||
                            !jobObject.room)
                    ) {
        ...
        /**@return {number}*/
        function GetDesiredLeftoverResource(
            structureType,
            resourceType,
            storage
        ) {
            ...
            const result = GenericJobAction(creep, roomJob, {
                ...
                JobStatus: function (jobObject) {
                    if (jobObject.mineralAmount === 0) {
                        ...
                        // fetch - drop minerals in nearby container
                        return SHOULD_FETCH;
                    } else {
                        // action not done yet
                        return SHOULD_ACT;
        ...
        function JobFillTerminal(creep, roomJob) {
            ...
            const result = GenericJobAction(creep, roomJob, {
                ...
                JobStatus: function (terminal) {
                    ...
                    storageResourceAmount = storage.store.getUsedCapacity(
                        resourceType
                    );
                    if (!creep.store.getUsedCapacity(resourceType)) {
                        if (
                            (storageResourceAmount <= Low &&
                                resourceType === RESOURCE_ENERGY) || // low resource in storage abort only if energy
                            ...
                            return JOB_IS_DONE;
                        } else if (!creep.store.getUsedCapacity(resourceType)) {
                            // fetch
                            return SHOULD_FETCH;
                        }
                    } else {
        ...
        function JobFillFactory(creep, roomJob) {
            ...
            const result = GenericJobAction(creep, roomJob, {
                /**@return {int}*/
                JobStatus: function (jobObject) {
                    const usedCapacity = jobObject.store.getUsedCapacity(
                        resourceType
                    );
                    if (
                        (resourceType === RESOURCE_ENERGY &&
                            usedCapacity >= Util.FACTORY_TARGET_ENERGY) ||
                        (resourceType !== RESOURCE_ENERGY &&
                            usedCapacity >= Util.FACTORY_TARGET_RESOURCE) ||
                        (creep.store.getUsedCapacity(resourceType) === 0 &&
                            !creep.room.storage.store.getUsedCapacity(
                                resourceType
                            ) &&
                            !creep.room.terminal.store.getUsedCapacity(
                                resourceType
                            ))
                    ) {
                        return JOB_IS_DONE;
                ...
                Fetch: function (fetchObject, jobObject) {
                    ...
                    return FetchResource(
                        ...
                        fetchObject,
                        resourceType,
                        max - jobObject.store.getUsedCapacity(resourceType)
                    );
                },
            });
            return result;
        ...
        function JobFillPowerSpawnEnergy(creep, roomJob) {
            const result = GenericJobAction(creep, roomJob, {
                ...
                Act: function (jobObject) {
                    ...
                    if (
                        result === OK &&
                        jobObject.store.getUsedCapacity(RESOURCE_ENERGY) > 4000
                    ) {
                        return JOB_IS_DONE;
                    }
                    return result;
            ...
            const result = GenericJobAction(creep, roomJob, {
                ...
                JobStatus: function (jobObject) {
                    if (jobObject.store.getFreeCapacity(RESOURCE_POWER) === 0) {
                        return JOB_IS_DONE;
                    } else if (!creep.store.getUsedCapacity(RESOURCE_POWER)) {
                        // fetch
                        return SHOULD_FETCH;
                    } else {
                        // action not done yet
                ...
                IsJobDone: function (jobObject) {
                    if (
                        creep.store.getUsedCapacity(RESOURCE_POWER) +
                            jobObject.store.getUsedCapacity(RESOURCE_POWER) >=
                        jobObject.store.getCapacity(RESOURCE_POWER)
                    ) {
                        return JOB_IS_DONE;
                    } else {
                        return this.JobStatus(jobObject);
                ...
                Fetch: function (fetchObject, jobObject) {
                    return FetchResource(
                        ...
                        fetchObject,
                        RESOURCE_POWER,
                        jobObject.store.getFreeCapacity(RESOURCE_POWER)
                    );
                },
            });
            return result;
        ...
        function JobTagController(creep, roomJob) {
            const result = GenericFlagAction(creep, roomJob, {
                ...
                /**@return {int}*/
                Fetch: function (fetchObject, jobObject) {
                    let result = creep.signController(
                        fetchObject,
                        jobObject.name
                    );
                    if (result === OK) {
                        Util.InfoLog(
                            "ExecuteJobs",
                            "JobTagController",
                            "JobTagController done " +
                                creep.name +
                                " in " +
                                jobObject.pos.roomName +
                                " tag " +
                                jobObject.name
                        );
                        jobObject.remove();
        ...
        function JobGuardPosition(creep, roomJob) {
            const result = GenericFlagAction(creep, roomJob, {
                ...
                JobStatus: function (jobObject) {
                    if (
                        !jobObject.room ||
                        jobObject.room.name !== creep.pos.roomName
                    ) {
                        return SHOULD_ACT;
                    } else {
                        return SHOULD_FETCH;
                ...
                FindFetchObject: function (jobObject) {
                    const hostileCreep = creep.pos.findClosestByPath(
                        FIND_HOSTILE_CREEPS
                    );
                    if (
                        hostileCreep &&
                        (hostileCreep.pos.getRangeTo(creep.pos) < 6 ||
                            (creep.room.controller && creep.room.controller.my))
                    ) {
                        return hostileCreep;
                    } else {
                        if (jobObject) {
                            const structure = jobObject.pos.findInRange(
                                FIND_STRUCTURES,
                                0
                            )[0];
                            if (structure) {
                            ...
                            const hostileStructure = jobObject.pos.findClosestByPath(
                ...
                /**@return {int}*/
                Fetch: function (fetchObject, jobObject) {
                    if (jobObject !== fetchObject) {
                        // hostileCreep or hostileStructure
                        if (fetchObject.structureType === STRUCTURE_PORTAL) {
                            Util.InfoLog(
                                "ExecuteJobs",
                                "JobGuardPosition",
                                "entering portal! " + fetchObject.pos.roomName
                            );
                            return ERR_NOT_IN_RANGE;
                        }
                        return creep.attack(fetchObject);
                    } else if (
                        creep.pos.x === jobObject.pos.x &&
                        creep.pos.y === jobObject.pos.y &&
                        creep.pos.roomName === jobObject.pos.roomName
                    ) {
                        return OK; // when OK is returned FindFetchObject is checking each tick for new hostileCreeps
                    } else if (jobObject === fetchObject) {
                        // move to flag
                        return ERR_NOT_IN_RANGE;
                    }
                },
        ...
        function JobGuardGunnerPosition(creep, roomJob) {
            const result = GenericFlagAction(creep, roomJob, {
                ...
                Fetch: function (fetchObject, jobObject) {
                    if (jobObject !== fetchObject) {
                        // hostileCreep or structure
                        let result = creep.rangedAttack(fetchObject);
                        if (
                            result === OK &&
                            creep.pos.getRangeTo(fetchObject) > 1
                        ) {
        ...
        function JobGuardMedicPosition(creep, roomJob) {
            const result = GenericFlagAction(creep, roomJob, {
                ...
                /**@return {object} @return {undefined}*/
                FindFetchObject: function (jobObject) {
                    const woundedCreep = creep.pos.findClosestByPath(
                        FIND_MY_CREEPS,
                        {
                ...
                Fetch: function (fetchObject, jobObject) {
                    if (jobObject !== fetchObject) {
                        // woundedCreep
                        if (
                            Math.abs(creep.pos.x - fetchObject.pos.x) > 1 ||
                            Math.abs(creep.pos.y - fetchObject.pos.y) > 1
                        ) {
                    ...
                    if (!creep.memory.HealthCheck) {
                        if (
                            ...
                            ((creep.name.startsWith("C") ||
                                creep.name.startsWith("R")) &&
                                creep.ticksToLive > 550)
                        ) {
                            ...
                            Util.Info(
                                ...
                                "JobMoveToPosition",
                                creep.name +
                                    " committed suicide ticksToLive " +
                                    creep.ticksToLive
                            );
                            new RoomVisual(creep.pos.roomName).text(
                                "💀",
                                creep.pos
                            );
                            creep.suicide();
            ...
            const result = GenericFlagAction(creep, roomJob, {
                ...
                JobStatus: function (jobObject) {
                    if (!creep.memory.Mineral) {
                        creep.memory.Mineral = jobObject.name
                            .split(/[-]+/)
                            .filter(function (e) {
                                return e;
                            })[1];
                        if (!lab) {
                            lab = jobObject.pos.findInRange(
                                FIND_MY_STRUCTURES,
                                0,
                                {
                                    filter: function (lab) {
                            ...
                            if (!lab) {
                                // lab does not exist - delete flag and remove job
                                jobObject.remove();
                                Util.ErrorLog(
                                    "ExecuteJobs",
                                    "JobFillLabMineral",
                    ...
                    if (lab.store.getFreeCapacity(creep.memory.Mineral) < 500) {
                        ...
                        creep.store.getUsedCapacity(creep.memory.Mineral) > 0
                    ) {
                ...
                IsJobDone: function (jobObject) {
                    if (
                        lab.store.getFreeCapacity(creep.memory.Mineral) -
                            creep.store.getUsedCapacity(creep.memory.Mineral) <=
                        0
                    ) {
                        // predict
                        return JOB_IS_DONE;
                    ...
                    return jobObject.room.find(FIND_STRUCTURES, {
                        filter: function (s) {
                            return (
                                ...
                                s.store.getUsedCapacity(creep.memory.Mineral) >
                                    0
                            );
                        },
                    })[0];
        ...
        function JobAttackPowerBank(creep, roomJob) {
            const result = GenericFlagAction(creep, roomJob, {
                ...
                Act: function (jobObject) {
                    if (!jobObject.room) {
                        // room is invisible - not in the room yet
                        return ERR_NOT_IN_RANGE;
                    }
                    let powerBank;
                    if (creep.memory.PowerBankId) {
                        powerBank = Game.getObjectById(
                            creep.memory.PowerBankId
                        );
                    }
                    if (!powerBank) {
                    ...
                    if (creep.hitsMax - creep.hits > 600) {
                        ...
                        if (
                            powerBank.hits <
                            Util.GENERATE_TRANSPORTER_WHEN_POWERBANK_HITS_UNDER
                        ) {
                            if (
                                !powerBank.room.lookForAt(LOOK_FLAGS, 0, 0)[0]
                            ) {
                                Util.Info(
                                    "ExecuteJobs",
                                    "JobAttackPowerBank",
                                    "generate transport power flags " +
                                        creep.name +
                                        " " +
                                        jobObject.name +
                                        " hits left " +
                                ...
                                const numOfTransporterFlags =
                                    powerBank.power /
                                    Util.TRANSPORTER_MAX_CARRY;
                                for (
                                    let i = 0;
                                    i < numOfTransporterFlags;
                                    i++
                                ) {
                                    jobObject.room.createFlag(
                                        i,
                                        0,
                                        i +
                                            "_getPower_" +
                        ...
                        const powerResource = jobObject.pos.findInRange(
                            FIND_DROPPED_RESOURCES,
                            1,
                            {
                                filter: function (power) {
                                    return (
                                        power.resourceType === RESOURCE_POWER
                                    );
                                },
                            }
                        )[0];
                        if (powerResource) {
                            Util.Info(
                                ...
                                "done " +
                                    ...
                                    jobObject.name +
                                    " power " +
                                    powerResource.amount
                            );
                        } else {
                            Util.Info(
                                "ExecuteJobs",
                                "JobAttackPowerBank",
                                "done " + creep.name + " " + jobObject.name
                            );
                        }
                        jobObject.remove();
                        result = JOB_IS_DONE;
        ...
        function JobMedicPowerBank(creep, roomJob) {
            const result = GenericFlagAction(creep, roomJob, {
                ...
                JobStatus: function (jobObject) {
                    if (
                        !jobObject.room ||
                        jobObject.pos.roomName !== creep.pos.roomName
                    ) {
                        // invisible
                        return SHOULD_ACT;
                    } else {
                        ...
                        if (!powerBank) {
                            powerBank = jobObject.pos.lookFor(
                                LOOK_STRUCTURES
                            )[0];
                            if (powerBank) {
                                creep.memory.PowerBankId = powerBank.id;
                ...
                FindFetchObject: function (jobObject) {
                    if (
                        creep.memory.PrimaryHealerTarget &&
                        Game.creeps[creep.memory.PrimaryHealerTarget] &&
                        Game.creeps[creep.memory.PrimaryHealerTarget].hits <
                            Game.creeps[creep.memory.PrimaryHealerTarget]
                                .hitsMax
                    ) {
                        ...
                        for (const woundedCreepKey in woundedCreeps) {
                            ...
                            for (const healerCreepKey in healerCreeps) {
                                const healerCreep =
                                    healerCreeps[healerCreepKey];
                                if (
                                    healerCreep.memory.PrimaryHealerTarget ===
                                    woundedCreep.name
                                ) {
                            ...
                            if (
                                !mostWoundedCreep ||
                                mostWoundedCreep.hitsMax -
                                    mostWoundedCreep.hits <
                                    woundedCreep.hitsMax - woundedCreep.hits
                            ) {
                ...
                Fetch: function (fetchObject, jobObject) {
                    if (fetchObject === jobObject) {
                        ...
                        if (
                            result !== OK ||
                            creep.getActiveBodyparts(HEAL) * 12 +
                                fetchObject.hits >=
                                fetchObject.hitsMax
                        ) {
                            // predict that creep is fully healed
                            return result;
                        } else {
                            return ERR_BUSY;
                        }
        ...
        function JobTransportPowerBank(creep, roomJob) {
            let result = GenericFlagAction(creep, roomJob, {
                ...
                Act: function (jobObject) {
                    ...
                    if (powerBank) {
                        // no powerResource on ground or powerRuin and in range to powerBank
                    ...
                    let powerTarget = jobObject.room.find(FIND_RUINS, {
                        filter: function (ruin) {
                            return (
                                ruin.store.getUsedCapacity(RESOURCE_POWER) > 0
                            );
                        },
                    })[0];
                    let result;
                    if (powerTarget) {
                        ...
                        powerTarget = jobObject.room.find(
                            FIND_DROPPED_RESOURCES,
                            {
                                filter: function (s) {
                    ...
                    if (result === ERR_NOT_IN_RANGE) {
                        ...
                        Util.Info(
                            "ExecuteJobs",
                            "JobTransportPowerBank",
                            "removing powerbank flag because last power has been picked up! " +
                                jobObject.pos.roomName
                        );
                        jobObject.remove();
                        return JOB_IS_DONE;
                ...
                Act: function (jobObject) {
                    if (!jobObject.room) {
                        ...
                        if (!deposit) {
                            ...
                            if (deposit) {
                                ...
                                Util.ErrorLog(
                                    ...
                                    "JobHarvestDeposit",
                                    creep.name +
                                        " no deposit found removed deposit flag in " +
                                        jobObject.pos.roomName
                                );
                                jobObject.remove();
                            }
                        ...
                        if (deposit && deposit.cooldown === 0) {
                            ...
                            deposit &&
                            deposit.lastCooldown >=
                                Util.DEPOSIT_MAX_LAST_COOLDOWN
                        ) {
                            Util.InfoLog(
                                "ExecuteJobs",
                                "JobHarvestDeposit",
        ...
        function JobDefendReserved(creep, roomJob) {
            const result = GenericFlagAction(creep, roomJob, {
                 ...
                 * @return {undefined}*/
                FindFetchObject: function (jobObject) {
                    const closestHostile = creep.pos.findClosestByPath(
                        FIND_HOSTILE_CREEPS
                    );
                    if (
                        closestHostile &&
                        (closestHostile.pos.getRangeTo(creep.pos) < 6 ||
                            (creep.room.controller && creep.room.controller.my))
                    ) {
                        return closestHostile;
                    } else {
                        const closestHostileStructure = creep.pos.findClosestByPath(
                            FIND_HOSTILE_STRUCTURES
                        );
        ...
        function FindAndRemoveMaxCreeps(jobRoomName, creepName) {
            ...
            if (
                ...
                // creep was not found in the expected room, now search all rooms for the creepName to remove
                ...
                for (const memRoomKey in Memory.MemRooms) {
                    // search for room with the creep
                    if (
                ...
                Util.ErrorLog(
                    "ExecuteJobs",
                    "FindAndRemoveMaxCreeps",
                    "could not find creep " +
                        creepName +
                        " in MaxCreeps memory last job change was in room " +
                        jobRoomName +
                        " creepType " +
                        creepType
                );
        ...
        function GenericFlagAction(creep, roomJob, actionFunctions) {
            ...
            const nearbyHostileCreeps = creep.pos.findInRange(
                FIND_HOSTILE_CREEPS,
                7
            );
            if (nearbyHostileCreeps.length > 0) {
                const hostileActionResult = CreepHostileAction(
                    creep,
                    nearbyHostileCreeps,
                    roomJob
                );
        ...
        function GenericAction(creep, roomJob, actionFunctions, targetObj) {
            ...
            if (!targetObj) {
                ...
                if (jobStatus === SHOULD_ACT) {
                    // act
                    result = actionFunctions.Act(targetObj);
                    if (result === ERR_NOT_IN_RANGE) {
                        if (
                            creep.pos.x !== targetObj.pos.x ||
                            creep.pos.y !== targetObj.pos.y ||
                            creep.pos.roomName !== targetObj.pos.roomName
                        ) {
                            result = Move(
                                creep,
                                targetObj,
                ...
                if (jobStatus === SHOULD_FETCH) {
                    // fetch immediately after maybe a successful Act that is not done
                    ...
                    if (!fetchObject) {
                        fetchObject = actionFunctions.FindFetchObject(
                            targetObj
                        );
                        if (!fetchObject) {
        ...
        function FindFetchResource(creep, jobObject, resourceToFetch) {
            let energySupply = FindClosestResourceInRoom(
                creep,
                jobObject.room,
                resourceToFetch,
                jobObject
        ...
        /**@return {object}
         * @return {undefined}*/
        function FindClosestResourceInRoom(
            creep,
            room,
            resourceToFetch,
            jobObject
            ...
            if (creep.memory.ResourceSupply) {
                resourceSupply = Game.getObjectById(
                    creep.memory.ResourceSupply
                ); // closest link then container then droppedRes then storage
                // if the saved resourceSupply does not have any energy then remove it to make way for a new search
                if (
                    !resourceSupply ||
                    !resourceSupply.store ||
                    !resourceSupply.store.getUsedCapacity(resourceToFetch)
                ) {
                    resourceSupply = undefined;
                    creep.memory.ResourceSupply = undefined;
                } else if (
                    resourceSupply &&
                    resourceSupply.structureType === STRUCTURE_STORAGE &&
                    creep.pos.roomName === jobObject.pos.roomName &&
                    creep.pos.getRangeTo(jobObject.pos) > 2
                ) {
                    // creep should have a chance at finding stores that are closer
                    creep.memory.ResourceSupply = undefined;
                }
            }
            if (!resourceSupply) {
                // creep memory had nothing stored
                        ...
                        return (
                            ((s.structureType === STRUCTURE_CONTAINER &&
                                (!creep.name.startsWith("T") ||
                                    s.id !==
                                        Memory.MemRooms[room.name]
                                            .CtrlConId)) || // extra check to deny controller containers as an energy source if creep is a Transfer creep
                                s.structureType === STRUCTURE_STORAGE ||
                                s.structureType === STRUCTURE_LINK ||
                                (jobObject.structureType !==
                                    STRUCTURE_TERMINAL &&
                                    s.structureType === STRUCTURE_TERMINAL)) &&
                            (s.store.getUsedCapacity(resourceToFetch) >= 200 ||
                                (resourceToFetch !== RESOURCE_ENERGY &&
                                    s.store.getUsedCapacity(resourceToFetch) >
                                        0))
                        );
                    },
                });
                resourceSupplies = resourceSupplies.concat(
                    room.find(FIND_DROPPED_RESOURCES, {
                        filter: function (d) {
                            return (
                                d.resourceType === resourceToFetch &&
                                d.amount >= 50
                            );
                        },
                    })
                ...
                resourceSupplies = resourceSupplies.concat(
                    room.find(FIND_TOMBSTONES, {
                        filter: function (t) {
                            return (
                                t.store.getUsedCapacity(resourceToFetch) >= 30
                            );
                        },
                    })
                );
                resourceSupplies = resourceSupplies.concat(
                    room.find(FIND_RUINS, {
                        filter: function (r) {
                            return (
                                r.store.getUsedCapacity(resourceToFetch) >= 30
                            );
                        },
                    })
                );
                ...
                for (let i = 0; i < resourceSupplies.length; i++) {
                    let distance = Math.sqrt(
                        Math.pow(
                            resourceSupplies[i].pos.x - jobObject.pos.x,
                            2
                        ) +
                            Math.pow(
                                resourceSupplies[i].pos.y - jobObject.pos.y,
                                2
                            )
                    );
                    if (
                        resourceSupplies[i].structureType === STRUCTURE_TERMINAL
                    ) {
                        distance += 1000;
                    } else if (
                        resourceSupplies[i].structureType === STRUCTURE_LINK
                    ) {
                        // prefer links over other stores
                        distance -= 3;
                    } else if (!resourceSupplies[i].structureType) {
                        // drop, tombstone or ruin is more important to pick up
                        distance -= 5;
                        if (
                            (resourceSupplies[i].store &&
                                resourceSupplies[i].store.getUsedCapacity(
                                    resourceToFetch
                                ) > 1000) ||
                            resourceSupplies[i].amount > 1000
                        ) {
        ...
        function HandleCreepBoost(
            creep,
            jobObject,
            boostingMineral,
            bodyPartToBoost,
            ticksToLiveToBoost = 1300,
            ticksToLiveToUnBoost = 100
        ) {
            if (
                !creep.memory.Boost ||
                creep.memory.Boost[bodyPartToBoost] !== "-"
            ) {
                // only look once - after that it is too late
                if (
                    creep.ticksToLive > ticksToLiveToBoost &&
                    (!creep.memory.Boost ||
                        (creep.memory.Boost &&
                            !creep.memory.Boost[bodyPartToBoost]))
                ) {
                    const labThatCanBoost = FindLabThatCanBoost(
                        creep,
                        jobObject,
                        boostingMineral,
                        bodyPartToBoost
                    ...
                    creep.memory.Boost &&
                    creep.memory.Boost[bodyPartToBoost] &&
                    creep.ticksToLive < ticksToLiveToUnBoost
                ) {
                    // unboost the creep before the mineral is lost
        ...
        function FindLabThatCanBoost(
            ...
            if (!creep.memory.Boost || !creep.memory.Boost[bodyTypeToBoost]) {
                const activeBodyPartsToBoost = creep.getActiveBodyparts(
                    bodyTypeToBoost
                );
                const labThatCanBoost = jobObject.room.find(
                    FIND_MY_STRUCTURES,
                    {
                        filter: function (lab) {
                            return (
                                ...
                                lab.store.getUsedCapacity(mineral) >=
                                    activeBodyPartsToBoost * 30 &&
                                lab.store.getUsedCapacity(RESOURCE_ENERGY) >=
                                    activeBodyPartsToBoost * 20
                            );
                        },
                    }
                )[0];
        ...
        function FetchResource(
            ...
            fetchObject,
            resourceToFetch,
            max = -1 /*withdraw to max carry capacity when -1 else withdraw until 'max' capacity is reached*/
        ) {
            let result;
            if (fetchObject.amount) {
                // identified as dropped resource - pickup
                if (max === -1) {
                ...
                if (
                    creep.store.getUsedCapacity(resourceToFetch) !==
                    creep.store.getUsedCapacity()
                ) {
                    ...
                    if (
                        fetchObject.store.getUsedCapacity(resourceToFetch) < max
                    ) {
                        if (
                            creep.store.getFreeCapacity() >
                            fetchObject.store.getUsedCapacity(resourceToFetch)
                        ) {
                            result = creep.withdraw(
                                fetchObject,
                                resourceToFetch,
                                fetchObject.store.getUsedCapacity(
                                    resourceToFetch
                                )
                            );
                        } else {
            ...
            if (result === ERR_FULL) {
                // creep store is full with anything other than resourceToFetch - get rid of it asap
                if (
                    fetchObject.store &&
                    fetchObject.store.getFreeCapacity(resourceToFetch) &&
                    fetchObject.structureType !== STRUCTURE_LINK
                ) {
                    for (const resourceType in creep.store) {
                        if (
                            creep.store.getUsedCapacity(resourceType) &&
                            resourceType !== resourceToFetch
                        ) {
        ...
        /**@return {number}*/
        function DepositCreepStore(
            creep,
            storeToFillObject,
            storeToEmptyObject = undefined,
            resourceTypeToKeep = undefined
        ) {
            if (
                creep.pos.roomName !== storeToFillObject.pos.roomName ||
                creep.pos.getRangeTo(storeToFillObject.pos) > 1
            ) {
                return ERR_NOT_IN_RANGE;
            }
            let result = ERR_NO_RESULT_FOUND;
            ...
            for (const resourceType in creep.store) {
                if (
                    creep.store.getUsedCapacity(resourceType) > 0 &&
                    resourceType !== resourceTypeToKeep
                ) {
                    if (countResources === 0) {
                        transferredAmount = creep.store.getUsedCapacity(
                            resourceType
                        );
                        result = creep.transfer(
                            storeToFillObject,
            ...
            if (
                result === OK &&
                countResources === 1 &&
                !creep.name.startsWith("H") &&
                (!storeToEmptyObject || // if there is a store to empty then look at how much the store has left and set JOB_IS_DONE if that store is empty
                    ((storeToEmptyObject.structureType ===
                        STRUCTURE_CONTAINER ||
                        storeToEmptyObject.structureType ===
                            STRUCTURE_STORAGE) &&
                        storeToEmptyObject.store.getUsedCapacity(
                            resourceTypeToKeep
                        ) < 500) ||
                    (storeToEmptyObject.structureType === STRUCTURE_LINK &&
                        storeToEmptyObject.store.getUsedCapacity(
                            resourceTypeToKeep
                        ) < 500) ||
                    (storeToEmptyObject.structureType === STRUCTURE_TERMINAL &&
                        resourceTypeToKeep === RESOURCE_ENERGY &&
                        storeToEmptyObject.store.getUsedCapacity(
                            resourceTypeToKeep
                        ) <= 120000 &&
                        storeToEmptyObject.room.storage.store.getUsedCapacity(
                            resourceTypeToKeep
                        ) >= 5000))
            ) {
                result = JOB_IS_DONE;
                ...
                result === ERR_NOT_IN_RANGE ||
                (result === OK && countResources <= 1)
            ) {
            } else if (result === OK && countResources > 1) {
                // if there are more to be transferred then set creep to busy
                result = ERR_BUSY;
            } else {
                let errorMessage =
                    "unexpected result! " +
                    ...
                    creep.store.getUsedCapacity() +
                    " storeToFill(" +
                    storeToFillObject.pos.x +
                    "," +
                    storeToFillObject.pos.y +
                    "," +
                    storeToFillObject.pos.roomName +
                    ")" +
                    (storeToEmptyObject
                        ? " storeToEmptyObject " + storeToEmptyObject + " "
                        : " ") +
                    (resourceTypeToKeep
                        ? " resourceTypeToKeep " + resourceTypeToKeep
                        : "");
                if (result === ERR_FULL) {
                    Util.ErrorLog(
                        "ExecuteJobs",
        ...
        function FindClosestFreeStore(
            creep,
            maxMoveRange = 0,
            resourceAmountToStore = 1 /*filters stores that are not large enough*/,
            resourceTypeToStore = undefined /*if energy then take link*/
        ) {
            let closestFreeStore = Game.getObjectById(
                creep.memory.ClosestFreeStoreId
            );
            if (closestFreeStore) {
                if (
                    closestFreeStore.store.getFreeCapacity() <
                        resourceAmountToStore ||
                    (maxMoveRange > 0 &&
                        creep.pos.getRangeTo(closestFreeStore) > maxMoveRange)
                ) {
            ...
            if (!closestFreeStore) {
                if (maxMoveRange > 0) {
                    const closestFreeStores = creep.pos.findInRange(
                        FIND_STRUCTURES,
                        maxMoveRange,
                        {
                            filter: function (s) {
                    ...
                    if (closestFreeStore) {
                        let bestRange = closestFreeStore.pos.getRangeTo(
                            creep.pos
                        );
                        for (const closestFreeStoreKey in closestFreeStores) {
                            if (
                                closestFreeStores[closestFreeStoreKey].id !==
                                closestFreeStore.id
                            ) {
                                if (
                                    resourceTypeToStore === RESOURCE_ENERGY &&
                                    closestFreeStores[closestFreeStoreKey]
                                        .structureType === STRUCTURE_LINK
                                ) {
                                    // if the type to store is energy then try and prioritize links
                                    closestFreeStore =
                                        closestFreeStores[closestFreeStoreKey];
                                    break;
                                }
                                const range = closestFreeStores[
                                    closestFreeStoreKey
                                ].pos.getRangeTo(creep.pos);
                                if (range < bestRange) {
                                    closestFreeStore =
                                        closestFreeStores[closestFreeStoreKey];
                                    bestRange = range;
                    ...
                    closestFreeStore = creep.pos.findClosestByPath(
                        FIND_STRUCTURES,
                        {
                            filter: function (s) {
                                return (
                ...
                if (!closestFreeStore && maxMoveRange === 0) {
                    // closestFreeStore still not found - look in nearest room for a storage that is free
                    Util.Info(
                        "ExecuteJobs",
                        "FindClosestFreeStore",
                        "not found, looking in other rooms for a storage"
                    );
                    let closestRoom;
                    ...
                    for (const gameRoomKey in Game.rooms) {
                        ...
                        if (
                            ...
                            gameRoom.storage &&
                            gameRoom.storage.store.getFreeCapacity() > 0 &&
                    ...
                    if (closestRoom) {
                        ...
                        Util.Info(
                            ...
                            creep.name +
                                " in " +
                                creep.pos.roomName +
                                " found closest available storage in " +
                                closestFreeStore.pos.roomName
                        );
                    }
        ...
        function GunnerHostileAction(creep, hostileCreeps) {
            ...
            if (hostileCreep) {
                ...
                if (
                    hostileCreep.getActiveBodyparts(ATTACK) &&
                    creep.pos.getRangeTo(hostileCreep) <= 2
                ) {
        ...
        function FindClosestHostileCreep(creep, hostileCreeps) {
            ...
            let hostileCreep;
            const nearbyHostileCreepsWithAttack = _.filter(
                hostileCreeps,
                function (hostileCreep) {
                    return (
                        hostileCreep.getActiveBodyparts(ATTACK) ||
            ...
            if (nearbyHostileCreepsWithAttack.length > 0) {
                hostileCreep = creep.pos.findClosestByPath(
                    nearbyHostileCreepsWithAttack
                );
            }

            // HEAL
            if (!hostileCreep) {
                const nearbyHostileCreepsWithHeal = _.filter(
                    hostileCreeps,
                    function (hostileCreep) {
        ...
        function HealerHostileAction(creep, hostileCreeps) {
            const closestDamagedCreeps = creep.pos.findInRange(
                FIND_MY_CREEPS,
                6,
                {
                    filter: function (c) {
            ...
            if (closestDamagedCreeps.length > 0) {
                const closestDamagedCreep = creep.pos.findClosestByPath(
                    closestDamagedCreeps
                );
            ...
            const nearbyHostileCreepsWithAttack = _.filter(
                ...
                function (hostileCreep) {
                    return (
                        (hostileCreep.getActiveBodyparts(ATTACK) &&
                            hostileCreep.pos.inRangeTo(creep, 2)) ||
                        (hostileCreep.getActiveBodyparts(RANGED_ATTACK) &&
                            hostileCreep.pos.inRangeTo(creep, 3))
                    );
                }
            );
            if (nearbyHostileCreepsWithAttack.length > 0) {
        ...
        function CivilianHostileAction(creep, hostileCreeps) {
            const hostileCreepsWithAttack = _.filter(
                hostileCreeps,
                function (hostileCreep) {
                    return (
                        hostileCreep.getActiveBodyparts(ATTACK) ||
        ...
        function Flee(creep, hostiles) {
            ...
            for (const hostileCreepKey in hostiles) {
                ...
                if (
                    hostileCreep &&
                    hostileCreep.pos &&
                    hostileCreep.pos.getRangeTo(creep.pos) < closesHostileRange
                ) {
            ...
            const fleePath = PathFinder.search(creep.pos, hostilePositions, {
                flee: true,
            });
        ...
        function Move(
            ...
            if (from.roomName === to.roomName || creep.memory.onlyMoveTo) {
                ...
                // not in the same room - make a map in mem
                if (
                    creep.memory.ExitPosition &&
                    creep.memory.ExitPosition.roomName === creep.pos.roomName
                ) {
                    // movement between rooms should reuse path more
                    result = creep.moveByPath(creep.memory.CachedPath);
                    if (result !== OK && result !== ERR_TIRED) {
                        Util.Warning(
                            "ExecuteJobs",
                            "Move",
                            "using cached path failed " +
                                creep.name +
                                " " +
                                creep.pos.roomName +
                                " " +
                                result +
                                " cached Path: " +
                                JSON.stringify(creep.memory.CachedPath)
                        );
                ...
                if (result === ERR_NO_RESULT_FOUND) {
                    // calculate path
                    const length = Util.GenerateOuterRoomPath(
                        to.roomName,
                        from.roomName
                    ); // saves result in Memory.Paths
                    if (length !== -1) {
        ...
        function getInnerRoomPath(to, creep) {
            ...
            creep.memory.CachedPath = creep.room.findPath(
                creep.pos,
                exitPosition
            );
        ...
        function MoveAnalysis(to, from, creep, result, obj) {
            ...
            if (result === OK) {
                ...
                if (creep.pos.x === 0) {
                    // get away from room exits asap
                    result = TryMoveInDirection(
                        creep,
                        BOTTOM_RIGHT,
                        RIGHT,
                        TOP_RIGHT
                    ...
                    if (!creep.memory.MoveErrWait) {
                        // maybe wait a couple of ticks to see if the obstacle has disappeared
                        creep.memory.MoveErrWait = 1;
                        creep.memory.MoveErrLastWait = Game.time;
                        result = JOB_MOVING;
                    } else if (creep.memory.MoveErrWait < 10) {
                        const ticksSinceWaitingStart =
                            Game.time - creep.memory.MoveErrLastWait;
                        if (
                            creep.memory.MoveErrLastWait &&
                            ticksSinceWaitingStart > 30
                        ) {
                            // if the start of the wait time is more than 30 ticks away then reset it
                            Util.Info(
                                "ExecuteJobs",
                                "Move",
                                "move error reset time MoveErrWait " +
                                    creep.memory.MoveErrWait +
                                    " waited " +
                                    ticksSinceWaitingStart +
                                    " ticks " +
                                    result +
                                    " " +
                                    creep.name +
                        ...
                        if (from.roomName === to.roomName) {
                            ...
                            Util.Warning(
                                "ExecuteJobs",
                                "Move",
                                "move error multiple room MoveErrWait " +
                                    creep.memory.MoveErrWait +
                                    " " +
                                    result +
                                    " " +
                        ...
                        if (result === ERR_INVALID_TARGET) {
                            Util.ErrorLog(
                                ...
                                "Move",
                                creep.name +
                                    " ERR_INVALID_TARGET when moving " +
                                    creep.pos.roomName
                            );
                        }
