    ...
    run: function () {
        ...
        function RunPowerCreeps() {
            ...
            for (const powerCreepKey in Game.powerCreeps) {
                ...
                if (!powerCreep.shard || powerCreep.shard === Game.shard.name) {
                    // if the creep is in another shard then just skip
                    let result = NO_RESULT_FOUND;
                    const flagWithCreepName = _.filter(
                        powerCreepSpawnFlags,
                        function (flag) {
                            return flag.name === powerCreep.name;
                        }
                    )[0];
                    if (!flagWithCreepName && powerCreep.shard) {
                        // this creep does not have a flag and is spawned - despawn it
                        Util.InfoLog(
                            "PowerCreeps",
                            "PowerCreepsActions",
                            "no flag for " +
                        ...
                        (!powerCreep.spawnCooldownTime ||
                            powerCreep.spawnCooldownTime < Game.time)
                    ) {
                        // it has a flag but it is not spawned
                        const powerSpawn = flagWithCreepName.pos.lookFor(
                            LOOK_STRUCTURES,
                            {
                                filter: (s) => {
                                    return (
                        ...
                        if (powerSpawn) {
                            ...
                            Util.ErrorLog(
                                "PowerCreeps",
                                "PowerCreepsActions",
                                "removed flag, powerSpawn not found " +
                                    flagWithCreepName.name +
                                    " (" +
                                    flagWithCreepName.pos.x +
                                    "," +
                                    flagWithCreepName.pos.y +
                                    "," +
                                    flagWithCreepName.pos.roomName +
                                    ")"
                            );
                            flagWithCreepName.remove();
                        }
                    } else if (powerCreep.shard) {
                        // flag is found and the creep is spawned - do something
                        if (
                            powerCreep.memory.JobName &&
                            !powerCreep.memory.JobName.startsWith("Unemployed")
                        ) {
            ...
            switch (true) {
                ...
                default:
                    Util.ErrorLog(
                        "PowerCreeps",
                        "PowerCreepsActions",
                        "job not found " + jobName + " " + powerCreep.name
                    );
            }
            return result;
        }
        ...
        function FindPowerCreepsActions(powerCreep) {
            ...
            if (powerCreep.ticksToLive < 500) {
                // power creep needs to be renewed
                ...
                powerCreep.room.controller.my &&
                !powerCreep.room.controller.isPowerEnabled
            ) {
                // my room is not power enabled
                powerCreep.memory.JobName = "EnablePowerInRoom";
                result = EnablePowerInRoom(powerCreep);
                Util.Info(
                    "PowerCreeps",
                    "PowerCreepsActions",
                    "trying to EnablePowerInRoom " +
                        powerCreep.name +
                        " " +
                        powerCreep.pos.roomName
                );
            } else if (powerCreep.className === POWER_CLASS.OPERATOR) {
                // power creep is not too old and power is enabled in the room
                if (
                    !powerCreep.memory.OperateTerminalCooldown ||
                    !powerCreep.memory.RegenSource1Cooldown ||
                    !powerCreep.memory.RegenSource2Cooldown ||
                    !powerCreep.memory.OperateFactoryCooldown ||
                    !powerCreep.memory.RegenMineralCooldown
                ) {
                ...
                if (
                    ...
                    powerCreep.memory.OperateFactoryCooldown < Game.time &&
                    powerCreep.powers[PWR_OPERATE_FACTORY] &&
                    powerCreep.powers[PWR_OPERATE_FACTORY].cooldown === 0 &&
                    powerCreep.store.getUsedCapacity(RESOURCE_OPS) >= 100
                ) {
                    powerCreep.memory.JobName = "OperateFactory";
                    result = OperateFactory(powerCreep);
                } else if (
                    (powerCreep.memory.RegenSource1Cooldown < Game.time ||
                        powerCreep.memory.RegenSource2Cooldown < Game.time) &&
                    ...
                    powerCreep.memory.RegenMineralCooldown < Game.time &&
                    powerCreep.powers[PWR_REGEN_MINERAL] &&
                    powerCreep.powers[PWR_REGEN_MINERAL].cooldown === 0
                ) {
                    powerCreep.memory.JobName = "RegenMineral";
                    result = RegenMineral(powerCreep);
                } else if (
                    powerCreep.store.getUsedCapacity(RESOURCE_OPS) > 500 ||
                    powerCreep.store.getUsedCapacity(RESOURCE_OPS) ===
                        powerCreep.store.getCapacity()
                ) {
        ...
        function RenewPowerCreep(powerCreep) {
            ...
            if (powerCreep.memory.PowerSpawnOrBankId) {
                powerSpawnOrBank = Game.getObjectById(
                    powerCreep.memory.PowerSpawnOrBankId
                );
                if (
                    powerSpawnOrBank &&
                    powerSpawnOrBank.pos.roomName !== powerCreep.pos.roomName
                ) {
            ...
            if (!powerSpawnOrBank) {
                ...
                if (!powerSpawnOrBank) {
                    // look in other visible rooms for a renew source
                    ...
                    for (const gameRoomKey in Game.rooms) {
                        ...
                        const range = Game.map.getRoomLinearDistance(
                            powerCreep.pos.roomName,
                            powerSpawnOrBank.pos.roomName
                        );
                        if (range < bestRange) {
                            powerSpawnOrBank = s;
                            bestRange = range;
                ...
                if (powerSpawnOrBank) {
                    Util.Info(
                        ...
                        powerCreep.name +
                            ...
                            powerCreep.pos.roomName +
                            " trying to renew at " +
                            JSON.stringify(powerSpawnOrBank.pos)
                    );
        ...
        function OperateFactory(powerCreep) {
            // PWR_OPERATE_FACTORY
            let result;
            let factory;
        ...
        function RegenSource(powerCreep) {
            ...
            if (source1 && source1.effects) {
                for (const effectKey in source1.effects) {
                    ...
                    if (
                        effect.effect === PWR_REGEN_SOURCE &&
                        effect.ticksRemaining > 30
                    ) {
        ...
        function RegenMineral(powerCreep) {
            ...
            if (mineral && mineral.mineralAmount > 0) {
                ...
                powerCreep.memory.JobName = "Unemployed";
                powerCreep.memory.RegenMineralCooldown =
                    Game.time + mineral.ticksToRegeneration;
                Util.Info(
                    "PowerCreeps",
                    "RegenMineral",
                    powerCreep.name +
        ...
        function DepositOps(powerCreep) {
            let opsToDeposit =
                powerCreep.store.getUsedCapacity(RESOURCE_OPS) - 100;
            let result = powerCreep.transfer(
        ...
        function WithdrawOps(powerCreep) {
            let amountToWithdraw =
                300 - powerCreep.store.getUsedCapacity(RESOURCE_OPS);
            let result;
            let target;
            if (
                powerCreep.room.storage &&
                powerCreep.room.storage.store.getUsedCapacity(RESOURCE_OPS) > 0
            ) {
                target = powerCreep.room.storage;
            } else if (
                powerCreep.room.terminal &&
                powerCreep.room.terminal.store.getUsedCapacity(RESOURCE_OPS) > 0
            ) {
            ...
            if (target) {
                if (
                    amountToWithdraw >
                    target.store.getUsedCapacity(RESOURCE_OPS)
                ) {
                    amountToWithdraw = target.store.getUsedCapacity(
                        RESOURCE_OPS
                    );
                }
        ...
        function OperateTerminal(powerCreep) {
            ...
            if (!operateTerminalEffectExist) {
                ...
                Util.Info(
                    ...
                    powerCreep.name +
                        " on terminal in " +
                        powerCreep.room.terminal.pos.roomName
                );
                if (result === ERR_NOT_IN_RANGE) {
            ...
            for (const spawnId in powerCreep.memory.spawns) {
                ...
                if (spawn) {
                    ...
                    for (const effectKey in spawn.effects) {
                        ...
                        if (
                            effect.effect === PWR_OPERATE_SPAWN &&
                            effect.ticksRemaining > 30
                        ) {
                            spawnHasBuff = true;
                            break;
                    ...
                    if (!spawnHasBuff) {
                        ...
                        if (result === ERR_NOT_IN_RANGE) {
                            ...
                            powerCreep.memory.JobName = "Unemployed";
                            powerCreep.memory.spawns[spawn.id] =
                                Game.time + 1000; // add duration
                        }
                        break;
                    }
