...
const RoomDefense = {
  run: function (gameRoom) {
    ...
    if (anyTowerActionLastTick || Game.time % Util.GAME_TIME_MODULO_1 === 0) {
      ...
      if (
        !anyTowerAction &&
        (Game.time % Util.GAME_TIME_MODULO_4 === 0 || anyTowerActionLastTick)
      ) {
      ...
      const hostileTargets = gameRoom.find(FIND_HOSTILE_CREEPS, {
        filter: function (hostile) {
          return (
            ...
            hostile.pos.findInRange(FIND_STRUCTURES, 4).length >= 0 ||
        ...
        if (
          ...
          if (isBoosted && hostileTarget.owner.username !== "Invader") {
            ...
            if (
              ...
              Game.notify(
                "Safemode has been activated for room: " +
                  gameRoom.name +
            ...
            if (
              ...
              if (existingDefendFlags.length > 0) {
                numOfDefenderFlagsPlaced =
                  numOfDefenderFlagsPlaced + existingDefendFlags.length;
              } else {
                const nameOfFlag =
                  "defend_" +
                  gameRoom.name +
